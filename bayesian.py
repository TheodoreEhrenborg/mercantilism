...
def main(game_results=list(range(32)), trials=1e4,
         test_case=None, final_diff_exp=-10):
...
#   f.close()
    ...
    for i in range(1, len(game_results)):
        ...
        if base_2[-1] == 0:
            ...
            multiplicities[0] += 1
            # Note that losses come first, then wins, 2-ties, ..., n-ties
            weights[0] = 0
        else:
...
def integrate(function, args, npoints, lowers, uppers,
              excluded_lowers=None, excluded_uppers=None):
...
#            print( np.any(this_point - excluded_lowers) < 0 )
        if go_for_it or np.min(
                this_point[1:-1] - excluded_lowers) < 0 or np.min(excluded_uppers - this_point[1:-1]) < 0:
...
#                    all_game_results[i][ inverse - 1 ] = all_game_results[i][ inverse - 1 ] + 1
    ...
    if all_game_trials > 0:
        current_confidence = decibels(
            c_bayesian.main(
                game_results=all_game_results,
                trials=trials))
        trials_used = trials
        if abs(
                current_confidence) < 40:  # If there is at least a one in ten thousand chance,
            current_confidence = decibels(
                c_bayesian.main(
                    game_results=all_game_results,
                    trials=100 * trials))
...
#    comparisons[ (a,b) ] = (current_confidence, all_game_trials, all_game_time, sum_game_results )
    ...
    f.write(
        time.asctime() +
        ": The bayesian algorithm used " +
        str(trials_used) +
        " trials.\n")
    f.close()
    return (current_confidence, all_game_trials, all_game_time,
            sum_game_results, all_game_results, trials_used)
...
def get_results(comparisons):
    ...
    import random
    import math
    name = "Results/Readable/Bayesian_Readable_" + \
...
#    print(len(comparisons))
    ...
    for c in comparisons.keys():
        ...
        results_list.append(
            [title, num_trials, invader_score, fixed, invader, confidence])
    f.close()
    results_list.sort()
    name = "Results/Readable/Bayesian_Analysis_by_Fixed_" + \
        time.asctime() + " " + str(random.randrange(10**9)) + ".txt"
    name = name.replace(" ", "_")
    g_name = "Results/Readable/Bayesian_Summary_" + \
        time.asctime() + " " + str(random.randrange(10**9)) + ".txt"
    ...
    name = "Results/Readable/Bayesian_Analysis_by_Invader_" + \
        time.asctime() + " " + str(random.randrange(10**9)) + ".txt"
    name = name.replace(" ", "_")
    f = open(name, "a")
        ...
        if invader != previously_invading:
            if not first:
                ...
                g.write(
                    ...
                    "\n")
                g.write("Net score: " +
                        str(invader_wins - invader_losses) + "\n\n")
                invader_wins = 0
                invader_ties = 0
                invader_losses = 0
    ...
    g.write(
        "\n\n" +
        "The total number of trials for all comparisons is " +
        str(total_number_trials))
    f.close()
    g.close()
