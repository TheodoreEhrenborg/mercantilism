...
def main(game_results=list(range(32)), trials=1e4, test_case=None, final_diff_exp=-10):
    ...
    for e in range(final_diff_exp, 1):
        ...
        results += integrate(
            both,
            args=[compressed, weights, average, multiplicities],
            npoints=long(trials),
...
def integrate(
    function, args, npoints, lowers, uppers, excluded_lowers=None, excluded_uppers=None
):
    ...
    for i in range(npoints):
        ...
        if (
            go_for_it
            or np.min(this_point[1:-1] - excluded_lowers) < 0
            or np.min(excluded_uppers - this_point[1:-1]) < 0
        ):
...
def check_probability(algorithm_tuple, trials=1e4):
    ...
    if all_game_trials > 0:
        current_confidence = decibels(
            c_bayesian.main(game_results=all_game_results, trials=trials)
        )
        trials_used = trials
        if (
            abs(current_confidence) < 40
        ):  # If there is at least a one in ten thousand chance,
            current_confidence = decibels(
                c_bayesian.main(game_results=all_game_results, trials=100 * trials)
            )
...
def get_results(comparisons):
    ...
    name = (
        "Results/Readable/Bayesian_Readable_"
        + time.asctime()
        + " "
    ...
    results_list.sort()
    name = (
        "Results/Readable/Bayesian_Analysis_by_Fixed_"
        + time.asctime()
        + " "
        + str(random.randrange(10 ** 9))
        + ".txt"
    ...
    name = name.replace(" ", "_")
    g_name = (
        "Results/Readable/Bayesian_Summary_"
        + time.asctime()
        + " "
        + str(random.randrange(10 ** 9))
        + ".txt"
    ...
    name = (
        "Results/Readable/Bayesian_Analysis_by_Invader_"
        + time.asctime()
        + " "
        + str(random.randrange(10 ** 9))
        + ".txt"
